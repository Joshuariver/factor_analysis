...
df <- read_spss("data/01_수업/2. 요인분석-WLB.sav")
...
df.a <- subset(df,select = -성장균형04)

df.f3 <- principal(df.a,5,rotate = "varimax", scores = TRUE)
print(fa.sort(df.f3,polar=FALSE), digits = 3, cut = .3)


# 다음은 "성장균형05"를 제거하고 다시 verimax 회전을 적용한 요인분석 실시

df.b <- subset(df,select = -c(성장균형04,성장균형05))

df.f4 <- principal(df.b,5,rotate = "varimax", scores = TRUE)
print(fa.sort(df.f4,polar=FALSE), digits = 3, cut = .3)
